...
if page == "Задание 1":
    ...
    st.write(""" P0 = """ + str(p_0_min*10**(-6)) + " - " + str('{:.3}'.format(p_0_max*10**(-6))) + """ МПа""")
    st.write(""" tпп = """ + str(tpp) + """ C """)
    st.write(""" Pпп = """ + str('{:.3}'.format(ppp*10**(-6))) + """ МПа""")
...
if page == "Задание 2":
    ...
    plt.xlabel('s кДж/кг*К')
    plt.title("h - s диаграмма")
    plot_hs_stage_t([6.18, 6.22], [3250, 3380])
    plt.grid(True)
    st.pyplot(fig3)
...
if page == "Задание 3":
    ...
    etaoi = st.number_input('Введите внутренний КПД ЦВД ηол', value=0.790, max_value=1.000)
...
if page == "Задание на автомат":
    ...
    #первая собственная частота вращения ротора на абсолютно жестких опорах:
    P11 = ((1*3.14)**2)/((L)**2)*((EI/(m_rot/L_summ))**0.5)
    #Первая собственная частота вращения ротора как жесткого целого на упругих опорах:
    P21 = (2/(m_rot*2.5e-9))**0.5
    #первая критическая частота вращения ротора:
    P1 = (1/((1/P11**2)+(1/P21**2))**0.5)/(2*3.14)
    #Вторая собственная частота вращения ротора как жесткого целого на абсолютно жестких опорах:
    P12 = 4*P11
    #Вторая собственная частота вращения ротора как жесткого целого на упругих опорах:
    P22 = (3**0.5)*(L/L_summ)*P21
    #вторая критическая частота вращения ротора:
    P2 = (1/((1/P12**2)+(1/P22**2))**0.5)/(2*3.14)
    st.write("""Macca вала  m_val = %.2f  кг""" % m_val)
    st.write("""Macca диска регулирующей ступени md_reg = %.2f  кг""" % md_reg)
    st.write("""Macca диска нерегулирующей ступени md_nereg = %.2f  кг""" % md_nereg)
    st.write("""Macca ротора  m_rot = %.2f  кг""" % m_rot)
    st.write("""Первая критическая частота P1 = %.2f  1/с""" % P1)
    st.write("""Вторая критическая частота P2 = %.2f  1/с""" % P2)
    if P1> 22.5 and P1< 27.5:
      ...
      st.write("""Новая частота  P1_new = %.2f  кг""" % P1_new)
    else:
